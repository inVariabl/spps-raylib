...
static void HandlePortTravel(Player *player, World *world, int portIdx) {
    ...
    if (world->state.worldId == WORLD_JUDEA) {
        if (!player->guardClearedForShip) return;
        LoadWorld(world, WORLD_MALTA);
    } else {
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (!combat.active && !player.guardDialogueActive && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
            Ray ray;
            if (isFirstPerson) {
                ...
                if (TextIsEqual(npcName, "Pharisee")) {
                    if (player.questStates[1] == QUEST_NOT_STARTED) {
                        ...
                        player.questStates[1] = QUEST_ACTIVE;
                               ...
                               player.position.x == 75 && player.position.z == 250) {
        ...
        if (!combat.active &&
            Vector3Distance(player.lerpPosition, (Vector3){(float)player.target.x, (float)player.target.y, (float)player.target.z}) > 0.1f) {
            DrawCircle3D((Vector3){(float)player.target.x, 0.01f, (float)player.target.z},
                         0.5f, (Vector3){1, 0, 0}, 90.0f, Fade(YELLOW, 0.5f));
        }
        EndMode3D();
    ...
    for (int i = 0; i < SPRITE_COUNT; i++) {
        UnloadTexture(spriteDatabase[i]);
    }
